...
description: React useEffect avoidance guide. Prefer ref callbacks, event handlers with `flushSync`, CSS, and `useSyncExternalStore` over reactive effects. Use when reviewing or writing React components in `.tsx` / `.jsx` files, when you see a `useEffect` call, when discussing whether a side effect needs an effect, or when refactoring effect-heavy components.
---
...
Apply this skill when reviewing or writing React components in `.tsx` or `.jsx` files. Check for unnecessary `useEffect` usage and suggest alternatives.
